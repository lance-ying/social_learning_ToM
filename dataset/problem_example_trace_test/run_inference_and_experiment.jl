            ...
            while !PDDL.satisfy(domain_agent1, state_agent1, problem_agent1.goal)
                ...
                for g in 1:length(goals)
                    for state_idx in 1:length(initial_states)
                        ...
                        if T == -1
                            for val in 1:size(goal_probs_conditioned_dict[map_id][g_id][state_idx], 2)
                                if any(x -> x < 0.1, goal_probs_conditioned_dict[map_id][g_id][state_idx][:, val])
                                    T = val
                                    break
                                end
                            end
                        ...
                        Q_T = estimate_self_exploration_cost(
                            domain_render,
                            new_state,
                            problem_agent1.goal,
                        ...
                        observe_action_cost = action_cost[:observe] * max(T, 1)
                ...
                Q_not_observe = estimate_self_exploration_cost(
                    domain_render,
                    new_state,
                    problem_agent1.goal,
                    wizard_candidates,
