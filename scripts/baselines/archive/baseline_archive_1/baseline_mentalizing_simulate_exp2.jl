...
for (map_id, goal_list) in metadata
    ...
    for (i, goal_str) in enumerate(goal_list)
        ...
        max_timesteps = size(state_probs_conditioned_dict[map_id][g_id][s_id], 2)
        if observe == 0 || (observe < max_timesteps && any(x -> x > 0.9, state_probs_conditioned_dict[map_id][g_id][s_id][:, observe+1]))
