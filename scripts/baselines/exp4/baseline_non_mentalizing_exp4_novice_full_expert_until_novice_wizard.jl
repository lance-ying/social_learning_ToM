...
model_label = "rational_non_mentalizing_novice_full_expert_until_novice_wizard"
...
progress = Progress(total_iterations, desc="Processing RNM novice-full/expert-until-novice-wizard baseline: ")
...
function agent_cost_comparison(domain_render, domain_path, problem_dir, map_id,
                                observed_plan, action_cost, target_horizon=nothing)
    ...
    T = isnothing(target_horizon) ?
        observation_stop_horizon(observed_plan) :
        realized_observation_horizon(target_horizon, observed_plan)
...
for (map_id, agent_goals) in sort(collect(metadata), by=x->parse(Int, match(r"\d+", x[1]).match))
    ...
    for scenario in 1:2
        ...
        should_observe_agent3, T_agent3 = agent_cost_comparison(
            domain_render, domain_path, PROBLEM_DIR, map_id, observed_plan_agent3, action_cost, length(observed_plan_agent3))
        ...
        replay_trace_dict[map_key] = Dict(
            ...
            "stop_reason" => "novice_full_expert_until_novice_first_wizard_interaction_cost_comparison",
        )
        next!(progress)
    end
...
# Save results
output_filename = "step_dict_rnm_novice_full_expert_until_novice_wizard_exp4.json"
...
replay_trace_filename = "replay_trace_rnm_novice_full_expert_until_novice_wizard_exp4.json"
