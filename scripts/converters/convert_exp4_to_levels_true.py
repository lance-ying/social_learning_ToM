#!/usr/bin/env python3
"""
Convert ASCII maps from problem_exp4_true directory to TypeScript level files.
...
def generate_typescript_level_exp4(level_id, ascii_map, x_goals, y_goals, x_expertise, y_expertise, steps_remaining=50):
    """Generate TypeScript level file content with two agents (X→Z and Y→O) using agent2 and agent3"""
    ...
    # Generate movement configurations for Agent 2 (X) - was Agent 1
    agent2_movements = {}
    ...
    # Generate movement configurations for Agent 3 (Y) - was Agent 2
    agent3_movements = {}
    ...
    # Generate TypeScript content with agent2 and agent3
...
def main():
    ...
    # Define paths - using problem_exp4_true
    ...
    for ascii_file in ascii_files:
        ...
        try:
            ...
            print(f"  X Agent (→Agent 2) Goals: {x_goals} with expertise: {x_expertise}")
            print(f"  Y Agent (→Agent 3) Goals: {y_goals} with expertise: {y_expertise}")
    ...
    with open(metadata_file, 'w') as f:
        f.write("Level ID | X_Goals (Agent2) | X_Expertise | Y_Goals (Agent3) | Y_Expertise | Primary_Goal_Type\n")
        f.write("-" * 100 + "\n")
    ...
    print("\n✓ Phase 1 complete! TypeScript files created with empty paths for agent2 and agent3.")
