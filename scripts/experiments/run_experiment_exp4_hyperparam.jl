...
# ARGS[1] = inference data filename (e.g., "inference_exp4_hyperparam_temp0.3.jld2")
# ARGS[2] = threshold value (e.g., "0.1", "0.15", "0.2", "0.25")
# ARGS[3] = comma-separated map IDs (e.g., "sm211,sm221,sm341,sm611,sm421")
# ARGS[4] = comma-separated scenario IDs (e.g., "1,2")
inference_filename = length(ARGS) >= 1 ? ARGS[1] : "inference_data_exp4_012626.jld2"
threshold = length(ARGS) >= 2 ? parse(Float64, ARGS[2]) : 0.1
selected_maps = length(ARGS) >= 3 && !isempty(ARGS[3]) ? split(ARGS[3], ",") : String[]
selected_scenarios = length(ARGS) >= 4 && !isempty(ARGS[4]) ? [parse(Int, s) for s in split(ARGS[4], ",")] : [2]

# Extract temperature from inference filename for output naming
# Pattern: "inference_exp4_hyperparam_temp0.3.jld2" -> "0.3"
function extract_temp_from_filename(filename::String)
    m = match(r"temp(\d+\.?\d*)", filename)
    return m !== nothing ? m.captures[1] : "unknown"
end

temp_str = replace(extract_temp_from_filename(inference_filename), "." => "p")
thresh_str = replace(string(threshold), "." => "p")
...
# Open debug log file
debug_log_path = joinpath(OUTPUT_DIR, "debug_hyperparam_temp$(temp_str)_thresh$(thresh_str).txt")
...
data = load(joinpath(@__DIR__, "..", "..", "data", "inference", inference_filename))
goal_probs_conditioned_dict = data["goal"]
state_probs_conditioned_dict = data["state"]
possible_worlds = data["worlds"]

debug_println("=== Hyperparameter Search ===")
debug_println("Temperature: $(extract_temp_from_filename(inference_filename))")
debug_println("Threshold: $threshold")
debug_println("Inference file: $inference_filename")
debug_println("Selected maps: $(selected_maps)")
debug_println("Selected scenarios: $(selected_scenarios)\n")
    ...
    for scenario in 1:2
        ...
        while !PDDL.satisfy(domain, state, problem.goal)
            ...
            task_agent2 = Threads.@spawn begin
                ...
                if goal_probs_agent2[g, timestep_agent2] < threshold
                    continue
                end

                for i in 1:length(initial_states_agent2)
                    if state_probs_agent2[i, timestep_agent2] < threshold
                        continue
                    end
                    ...
                    if T == -1 || T > max_T_state || !T_from_state
                        ...
                        for j in 1:length(blue_wizards_agent2)
                            if state_probs_gi[j, T] > threshold
            ...
            task_agent3 = Threads.@spawn begin
                ...
                if goal_probs_agent3[g, timestep_agent3] < threshold
                    continue
                end

                for i in 1:length(initial_states_agent3)
                    if state_probs_agent3[i, timestep_agent3] < threshold
                        continue
                    end
            ...
            if best_action_idx == 1
                ...
                for j in 1:length(blue_wizards)
                    if state_probs_agent2[j, agent2_count + 1] > threshold
            ...
            elseif best_action_idx == 2
                ...
                # Simple wizard update: use state_probs directly (matching exp3_debug approach)
                for j in 1:length(blue_wizards)
                    if state_probs_agent3[j, agent3_count + 1] > threshold
                        push!(wizard_candicates, blue_wizards[j])
                    end
                end
            else
...
output_filename = "steps_dict_hyperparam_temp$(temp_str)_thresh$(thresh_str).json"
