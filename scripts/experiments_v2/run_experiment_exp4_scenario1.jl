...
CANONICAL_OUTPUT_DIR = joinpath(OUTPUT_DIR, "scenario1")
mkpath(CANONICAL_OUTPUT_DIR)
DETAIL_OUTPUT_DIR = joinpath(OUTPUT_DIR, "_wrapper_artifacts")
mkpath(DETAIL_OUTPUT_DIR)

# Open debug log file
debug_log_path = joinpath(DETAIL_OUTPUT_DIR, "$(output_prefix)_scenario1_debug.txt")
...
for (map_id, agent_goals) in metadata
    ...
    for scenario in 1:2
        ...
        function cached_exploration_cost(wizards)
            ...
            cost = try; estimate_self_exploration_cost(domain_render, new_state, problem_agent1.goal, wizards, action_cost); catch err; @warn "infeasible wizard set, treating Q as Inf" exception=err wizards=wizards; Inf; end
            lock(exploration_cost_cache_lock) do
        ...
        while !PDDL.satisfy(domain_agent1, state_agent1, problem_agent1.goal)
            ...
            task_agent2 = Threads.@spawn begin
                ...
                for i in 1:length(initial_states_agent2)
                    ...
                    remaining_T = T_from_state ? max(T - timestep_agent2, 1) : max(T - 1, 1)
            ...
            task_agent3 = Threads.@spawn begin
                ...
                for i in 1:length(initial_states_agent3)
                    ...
                    # Use REMAINING time to convergence, not total time
                    # This encourages continuing with an agent we've already started observing
                    remaining_T = T_from_state ? max(T - timestep_agent3, 1) : max(T - 1, 1)
                    obs_cost = action_cost[:observe] * remaining_T
                    total_cost = Q_T + obs_cost
            ...
            decision_label = best_action_idx == 1 ? "observe_agent2" : (best_action_idx == 2 ? "observe_agent3" : "stop")
            push!(decision_trace, Dict(
                "timestep" => t,
                "q_observe_agent2" => Q_observe_agent2,
                "q_observe_agent3" => Q_observe_agent3,
                "q_not_observe" => Q_not_observe,
                "gap_a2" => Q_not_observe - Q_observe_agent2,
                "gap_a3" => Q_not_observe - Q_observe_agent3,
                "min_gap" => Q_not_observe - min(Q_observe_agent2, Q_observe_agent3),
                "decision" => decision_label,
                "agent2_count_before" => agent2_count,
                "agent3_count_before" => agent3_count,
            ))
...
output_filename = "$(output_prefix)_scenario1.json"
...
replay_trace_filename = "$(output_prefix)_scenario1_replay_trace.json"
