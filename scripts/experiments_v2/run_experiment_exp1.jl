    ...
    while !PDDL.satisfy(domain_agent1, state_agent1, problem_agent1.goal)
        ...
        for g in 1:length(goals)
            ...
            for i in 1:length(initial_states)
                ...
                Q_T = try
                    estimate_self_exploration_cost(domain_render, new_state, problem_agent1.goal, new_wizard_candicates, action_cost)
                catch err
                    @warn "skipping infeasible hypothesis (g=$g, i=$i) for $map_key" exception=err
                    continue
                end
        ...
        Q_not_observe = try
            estimate_self_exploration_cost(domain_render, new_state, problem_agent1.goal, wizard_candicates, action_cost)
        catch err
            @warn "Q_not_observe infeasible for $map_key, treating as Inf" exception=err
            Inf
        end
