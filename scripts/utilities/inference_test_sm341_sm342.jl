...
# Only process these specific maps
maps_to_process = ["sm341", "sm342", "sm331", "sm332"]
...
for agent_name in agents_to_infer
    ...
    for (map_id, agent_goals) in metadata
        # Only process specified maps
        if map_id ∉ maps_to_process
            println("Skipping map: $map_id (not in test list)")
            continue
        end
        ...
        for scenario in 1:2
            ...
            agent_config = AgentConfig(
                ...
                replan_args = (
                    ...
                    prob_replan = 0, # Probability of replanning at each step
...
# Save to a test file
output_file = joinpath(@__DIR__, "..", "..", "data", "inference", "inference_data_exp3_test_sm341_sm342_sm331_sm332.jld2")
