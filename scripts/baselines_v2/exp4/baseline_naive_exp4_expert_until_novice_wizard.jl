...
model_label = "naive_observer_expert_only_until_expert_wizard"
...
progress = Progress(total_iterations, desc="Processing naive expert-only/until-novice-wizard baseline: ")
...
for (map_id, agent_goals) in sort(collect(metadata), by=x->parse(Int, match(r"\d+", x[1]).match))
    ...
    for scenario in 1:2
        ...
        replay_trace_dict[map_key] = Dict(
            ...
            "agent2_count" => agent2_count,
            "agent3_count" => agent3_count,
            "stop_reason" => "expert_only_until_expert_first_wizard_interaction",
        )
        next!(progress)
    end
...
# Save results
output_filename = "step_dict_naive_expert_only_until_expert_wizard_exp4.json"
...
replay_trace_filename = "replay_trace_naive_expert_only_until_expert_wizard_exp4.json"
