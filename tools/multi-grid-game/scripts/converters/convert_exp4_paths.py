#!/usr/bin/env python3
"""
Convert exp4 pathing data from pathing_exp4.json to movement paths compatible with multi-grid-game.
...
#     from pathfinder import find_path as existing_pathfinder
#     HAS_PATHFINDER = True
# except ImportError:
    ...
    for y, line in enumerate(lines):
        ...
        for x, char in enumerate(line):
            if char == "e":  # Empty wizard
                ...
                gem_counter += 1
            elif char == "X":  # Agent 1 start (maps to Agent 1 in current system)
                entities["start_pos"]["agent1"] = (x, y)
            elif char == "Y":  # Agent 2 start (maps to Agent 2 in current system)
                entities["start_pos"]["agent2"] = (x, y)
            elif (
                char == "M"
            ):  # Player position (legacy, use as Agent 1 fallback if X/Y not found)
                if "agent1" not in entities["start_pos"]:
                    entities["start_pos"]["agent1"] = (x, y)
...
def map_exp4_agent_to_current_agent(exp4_agent_name):
    ...
    exp4 agent2 -> current Agent 1 (X character)
    exp4 agent3 -> current Agent 2 (Y character)
    """
    if exp4_agent_name == "agent2":
        return 1  # Agent 1 in current system
...
def main():
    ...
    for level_id, scenarios in pathing_data.items():
        ...
        for scenario_name, agents in scenarios.items():
            ...
            for exp4_agent_name, agent_data in agents.items():
                # Map exp4 agent to current system agent
    ...
    print("Note: Keys are ignored as they don't exist in current system.")
    print("Barriers use existing amulet system, interactions are automatic.")
