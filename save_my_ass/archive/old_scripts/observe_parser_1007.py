...
if __name__ == "__main__":
    # Process files from pilot_100625+092925 directory
    print("\n" + "="*60)
    print("PROCESSING: pilot_100725+100625+092925")
    print("="*60)
    paths = list(Path("pilot_100725+100625+092925").glob("*.csv"))
    per_file, overall, dict_result, json_result = count_observes_and_means(paths)
    print("\nRESULT dictionary:")
    print(json_result)

    # Save dictionary to Python file
    output_dir = Path("processed_outputs")
    output_dir.mkdir(exist_ok=True)

    with open(output_dir / "results_dict_merged_new.py", "w") as f:
        f.write(f"# Generated observe statistics\n")
        f.write(f"results_dict = {repr(dict_result)}\n")

    print("\n" + "="*60)
    print("SAVED PYTHON FILE:")
    print("="*60)
    print(f"  - {output_dir / 'results_dict_merged_new.py'}")