            ...
            task_agent2 = Threads.@spawn begin
                ...
                for i in 1:length(initial_states_agent2)
                    ...
                    if T == -1 || T > max_T_state
                        # If T is invalid, use all wizards as candidates from FULL state
                        new_wizard_candicates = copy(blue_wizards)
                    else
                        # Get blue wizards from FULL state using indices from filtered state
                        ...
                        for j in 1:length(blue_wizards_agent2)
                            if state_probs_conditioned_dict["agent2"][map_id][scenario][g][i][j, T] > 0.1
                                push!(new_wizard_candicates, blue_wizards[j])  # Use wizard from FULL state!
                            end
                        end
                    end
