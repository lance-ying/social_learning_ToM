...
for (map_id, agent_goals) in metadata
    ...
    for scenario in 1:2
        ...
        while !PDDL.satisfy(domain, state, problem.goal)
            ...
            # Check which agents still have data available
            agent2_has_data = t < max_t_agent2
            agent3_has_data = t < max_t_agent3
            
            if !agent2_has_data && !agent3_has_data
                println("    -> Both agents out of inference data at t=$t")
                steps_dict[map_key] = Dict(
                    "t" => t,
            ...
            if !agent2_has_data
                println("    -> agent2 out of data, only considering agent3")
            end
            if !agent3_has_data
                println("    -> agent3 out of data, only considering agent2")
            end
            
            # Parallelize Q computation for agents with available data
            Q_observe_agent2 = Inf  # Default to Inf if no data
            total_probs_agent2 = 0.0
            
            task_agent2 = if agent2_has_data
                Threads.@spawn begin
            ...
            Q_observe_agent3 = Inf  # Default to Inf if no data
            total_probs_agent3 = 0.0
            
            task_agent3 = if agent3_has_data
                Threads.@spawn begin
            ...
            # Wait for parallel tasks to complete (only if they were started)
            if agent2_has_data
...
output_filename = "steps_dict_$(experiment_id)_optimized_new_stimuli.json"
