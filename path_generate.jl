...
metadata = JSON.parsefile(joinpath(@__DIR__, "dataset", "problems_exp3", "metadata.json"))
...
for stimulus_id in stimulus_ids
    ...
    for (agent_id, gem_numbers) in agent_goals
        ...
        # Create filtered problem for this agent
...
for (plan_id, plan_actions) in plan_dict
    # Create the filename using the new convention: {stimulus_id}_{agent_id}_plan.pddl
