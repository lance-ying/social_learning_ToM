#!/usr/bin/env python3
# json_to_report_ffill_movecalc.py (adapted for multi-NPC support)
import csv, json, re
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: uv run scripts/convert_json_to_csv.py <json_file> [output_dir]")
        print("   or: uv run scripts/convert_json_to_csv.py <json_directory> [output_dir]")
        sys.exit(1)
    ...
    if input_path.is_file() and input_path.suffix == ".json":
        ...
        if output_dir is None:
            output_dir = Path(input_path.stem + "_csvs")
        print(f"Converting {input_path} to CSV files in {output_dir}/")
        csv_files = build_report_files(str(input_path), str(output_dir))
        print(f"✓ Generated {len(csv_files)} CSV files in {output_dir}/")
    elif input_path.is_dir():
        # Directory of JSON files
        process_directory(str(input_path), str(output_dir) if output_dir else None)
    else:
        print(f"Error: Input must be a JSON file or directory containing JSON files")
        sys.exit(1)
