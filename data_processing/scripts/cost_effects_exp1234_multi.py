#!/usr/bin/env python3
"""
Paired effects plots for model-minus-human costs across experiments 1-4.

For each experiment and model, the plot shows per-level paired differences
(`model - human`) as semi-transparent points, along with the mean paired
difference and a bootstrap 95% confidence interval across matched levels.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create paired effects plots for reconstructed model and human costs."
    )
    parser.add_argument(
        "--metrics",
        nargs="+",
...
def plot_metric(ax: plt.Axes, matched_by_experiment: dict[str, tuple[dict[str, dict[str, float]], dict]], metric: str) -> None:
    ...
    for exp_idx, exp in enumerate(EXPERIMENTS):
        ...
        for model_idx, (_series_label, series_key) in enumerate(MODEL_SERIES):
            ...
            xpos = x[model_idx] + offsets[exp_idx]
            jitter = rng.uniform(-JITTER_WIDTH / 2, JITTER_WIDTH / 2, size=diffs.size)
            mean_val, ci_low, ci_high = bootstrap_mean_ci(diffs, rng)

            ax.scatter(
                np.full(diffs.size, xpos) + jitter,
                diffs,
                s=POINT_SIZE,
                color=color,
                alpha=POINT_ALPHA,
            ...
            ax.scatter(
                ...
                linewidths=0.7,
                zorder=4,
                label=exp_label if model_idx == 0 else None,
            )
...
def main() -> int:
    ...
    output_file = (
        ...
        if args.output_file
        else repo_root / "data_processing" / "outputs" / "plots" / "cost_effects_exp1234_multi.png"
    )
    ...
    fig.text(
        0.5,
        0.01,
        "Points show matched-level differences; large points and lines show mean difference with bootstrap 95% CI.",
        ha="center",
        va="bottom",
        fontsize=11,
        color="#333333",
    )
    plt.tight_layout(rect=(0, 0.04, 1, 1), w_pad=2.5)
