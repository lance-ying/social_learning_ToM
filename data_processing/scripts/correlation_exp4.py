#!/usr/bin/env python3
"""
Correlation analysis for EXP4: Compare model predictions with human data.
Based on correlation_exp3.py structure.
"""
import json
import numpy as np
...
def main(human_json_path: str, model_json_path: str, output_dir: str = None):
    """
    ...
    print("=" * 80)
    print("EXP4: Model Predictions vs Human Data")
    print("=" * 80)
    ...
    for human_level, human_data in sorted(human_dict.items()):
        ...
        if model_level in model_dict:
            ...
            print(f"  ✓ {human_level} → {model_level}: agent2: model={model_agent2}, human={human_mean_agent2:.2f} | agent3: model={model_agent3}, human={human_mean_agent3:.2f}")
        else:
    ...
    if len(arr_model_agent2) > 0:
        ...
        stats_path = output_dir / "correlation_stats_exp4.json"
        ...
        ax1.set_title(f'exp4 agent2 corr\nr = {corr_agent2:.3f}', fontsize=20)
        ax1.grid(False)
        ...
        plt.tight_layout()
        plot_path_agent2 = output_dir / 'correlation_scatterplot_agent2.png'
        ...
        ax2.set_title(f'exp4 agent3 corr\nr = {corr_agent3:.3f}', fontsize=20)
        ax2.grid(False)
        ...
        plt.tight_layout()
        plot_path_agent3 = output_dir / 'correlation_scatterplot_agent3.png'
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python correlation_exp4.py <human_json> <model_json> [output_dir]")
        print("\nExample:")
        print("  python correlation_exp4.py \\")
    ...
    main(human_file, model_file, output_directory)
