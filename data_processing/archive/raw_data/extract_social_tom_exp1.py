...
def build_report_files(json_path: str, out_dir: str) -> List[str]:
    ...
    for user_id, user_data in users.items():
            ...
            for batch_id, batch_data in (batches or {}).items():
                for idx, ev in enumerate((batch_data or {}).get("events", []) or []):
                    ...
                    rows.append({
                        ...
                        "NPC X": agent_pos.get("x"),
                        "NPC Y": agent_pos.get("y"),
                        "Steps Remaining": steps_remaining,
                        "NPC Path Type": path_type,
                        "NPC Current Path Index": path_index,
        ...
        out_path = Path(out_dir) / f"{str(user_id).replace('/', '_')}_pilot_101725.csv"
        with out_path.open("w", newline="", encoding="utf-8") as f:
            ...
            if rows:
                ...
                for level_id, gl in df.groupby("level_id", dropna=False):
                    ...
                    cols = ["Timestamp","Type","Player Action","Player X","Player Y","NPC X","NPC Y","Steps Remaining","NPC Path Type","NPC Current Path Index","Interaction Type","Interaction Details"]
                    gl = gl.sort_values(by=["timestamp_ms"]).reset_index(drop=True)

                    # Compute post-move positions for action rows when missing
                    last_px = last_py = last_ax = last_ay = None
                    for i in range(len(gl)):
                        px, py = gl.at[i, "Player X"], gl.at[i, "Player Y"]
                        ax, ay = gl.at[i, "NPC X"], gl.at[i, "NPC Y"]
                        if pd.notna(px): last_px = float(px)
                        if pd.notna(py): last_py = float(py)
                        if pd.notna(ax): last_ax = float(ax)
                        if pd.notna(ay): last_ay = float(ay)
                    ...
                    ffill_cols = ["Type","Player X","Player Y","NPC X","NPC Y","Steps Remaining","NPC Path Type","NPC Current Path Index"]
                    for c in ffill_cols:
                        if c in gl.columns:
...
if __name__ == "__main__":
    print("\n".join(build_report_files("pilot_101725.json", ".")))
